...
class BIMValidator {
    ...
    async compareElement(bimElement, actual) {
        const deviations = [];
    ...
    async batchCompare(elements, actuals) {
        const results = [];
    ...
    async detectMissingElements(bimElements, actualElements) {
    ...
    async detectDeviations(bimElement, actual) {
        const deviations = [];
        if (bimElement.dimensions && actual.dimensions) {
            deviations.push(...this.compareDimensions(bimElement.dimensions, actual.dimensions));
        }
    ...
    async calculateMatchPercent(bimElements, actualElements) {
    ...
    async generateComparisonReport(projectId, comparisons) {
    ...
    compareDimensions(design, actual) {
        const deviations = [];
        const params = ['width', 'height', 'depth'];
    ...
    compareProperties(design, actual) {
        const deviations = [];
    ...
    findMatchingId(element, actuals) {
        const match = actuals.find(a => a.type === element.type &&
            a.name === element.name);
        return match?.id || '';
    }
    categorizeDeviation(deviation, tolerance) {
    ...
    generateSummary(matchPercent, nonMatchingCount, missingCount) {
        if (matchPercent >= 95) {
    ...
    getComparisonHistory(elementId) {
        return this.comparisons.get(elementId) || [];
    }
}
exports.BIMValidator = BIMValidator;
//# sourceMappingURL=bim-validator.js.map