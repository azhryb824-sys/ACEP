...
exports.BusinessIntelligenceEngine = void 0;
const core_1 = require("@acep/core");
const uuid_1 = require("uuid");
const types_1 = require("./types");
class BusinessIntelligenceEngine extends core_1.BaseEngine {
    dataSources = new Map();
    dashboards = [];
    alerts = new Map();
    alertDefs = new Map();
    ...
    initializeDataSources() {
        for (const source of Object.values(types_1.DataSourceType)) {
            this.dataSources.set(source, []);
        }
    }
    initializeAlertDefinitions() {
        const defs = [
            {
                id: 'alert-profit-drop', name: 'Profit Drop', description: 'Profit drops below threshold',
                source: types_1.DataSourceType.Accounting,
            ...
            },
            {
                ...
                source: types_1.DataSourceType.Risk,
            ...
            },
            {
                ...
                source: types_1.DataSourceType.Projects,
            ...
            },
            {
                ...
                source: types_1.DataSourceType.Safety,
            ...
            },
            {
                ...
                source: types_1.DataSourceType.Quality,
            ...
            },
            {
                ...
                source: types_1.DataSourceType.Inventory,
    ...
    async collectData(sources) {
        const targets = sources || Object.values(types_1.DataSourceType);
    ...
    async collectFromSource(source) {
        const mockData = {
            id: (0, uuid_1.v4)(), source, collectedAt: new Date().toISOString(),
    ...
    async trackKPI(type, value, target) {
        ...
        if (status !== 'on_track') {
            const alert = {
                id: (0, uuid_1.v4)(), alertId: `kpi-${type}`, alertName: `KPI: ${type}`,
    ...
    async getCompanyHealth() {
        ...
        health.overall = Math.round((health.financial * 0.15 + health.projects * 0.15 + health.quality * 0.12 +
    ...
    async evaluateAlerts() {
        for (const def of this.alertDefs.values()) {
            ...
            for (const threshold of def.thresholds) {
                ...
                if (triggered) {
                    ...
                    const hasRecent = existingAlerts.some(a => a.alertId === def.id &&
                        !a.acknowledged &&
                        Date.now() - new Date(a.triggeredAt).getTime() < def.cooldownMinutes * 60000);
                    if (!hasRecent) {
                        const notification = {
                            id: (0, uuid_1.v4)(), alertId: def.id, alertName: def.name,
        ...
        if (allAlerts.length > 1000) {
            ...
            for (const r of toRemove)
                this.alerts.delete(r.id);
        }
    }
    async createDashboard(name, sections, filters) {
    ...
    async exportDashboard(format) {
        const url = `/exports/dashboard-${(0, uuid_1.v4)()}.${format}`;
        this.logger.info(`Dashboard exported as ${format}: ${url}`);
        return url;
    }
    async customizeDashboard(config) {
        this.config = { ...this.config, ...config };
        this.logger.info('Dashboard customized');
    }
    async getDashboardHistory(limit = 10) {
        return this.dashboards.slice(-limit);
    }
    async descriptiveAnalysis(metric, period) {
    ...
    async diagnosticAnalysis(metric, anomaly) {
        const rootCauses = [
    ...
    async predictiveAnalysis(metric, horizon) {
        return this.generatePrediction(metric, horizon);
    }
    async prescriptiveAnalysis(metric, target) {
    ...
    async createScenario(name, description, assumptions) {
        const baseRevenue = 50000000;
        const baseCost = 35000000;
        const scenario = {
            id: (0, uuid_1.v4)(), name, description, assumptions,
            scenarioA: {
    ...
    async compareScenarios(scenarioId) {
    ...
    async runScenario(scenarioId, variant, parameters) {
        const scenario = this.scenarios.find(s => s.id === scenarioId);
        if (!scenario)
            throw new Error(`Scenario ${scenarioId} not found`);
    ...
    async compareEntities(entityIds, metrics) {
        const config = {
    ...
    async getIndustryAverages(metrics) {
        const averages = {};
