...
import {
  ...
  ScenarioAnalysis, PredictionResult, ReportConfig, BenchmarkConfig, BenchmarkResult,
  ExecutiveQuery, ExecutiveResponse, DataSourceType, FinancialMetric, ProjectMetric,
  ResourceMetric, KPIType
} from './types';
...
export interface IBusinessIntelligenceEngine extends IEngine {
  ...
  trackKPI(type: KPIType, value: number, target: number): Promise<{ actual: number; target: number; variance: number; status: 'on_track' | 'at_risk' | 'behind' }>;
...
export interface IAnalyticsEngine {
  descriptiveAnalysis(metric: string, period: string): Promise<{ mean: number; median: number; stdDev: number; min: number; max: number; trend: string }>;
  diagnosticAnalysis(metric: string, anomaly: number): Promise<{ rootCauses: string[]; contributingFactors: string[]; correlation: Record<string, number> }>;
  predictiveAnalysis(metric: string, horizon: number): Promise<PredictionResult>;
  prescriptiveAnalysis(metric: string, target: number): Promise<{ recommendations: string[]; expectedImpact: number; confidence: number }>;
}
...
export interface IScenarioManager {
  ...
  compareScenarios(scenarioId: string): Promise<{ recommendation: string; comparison: Record<string, { a: number; b: number; c: number }> }>;
...
export interface IReportGenerator {
  ...
  getReportHistory(type?: string): Promise<{ id: string; type: string; format: string; generatedAt: string; url: string }[]>;
