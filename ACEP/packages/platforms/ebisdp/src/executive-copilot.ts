...
import {
  ExecutiveQuery, ExecutiveResponse, ExecutiveDashboard, EnterpriseHealthIndex,
  AlertNotification, ScenarioAnalysis, PredictionResult, FinancialMetric,
  ProjectMetric, DataSourceType
} from './types';
import { BusinessIntelligenceEngine } from './engine';

export class ExecutiveCopilot extends BaseEngine {
  private engine: BusinessIntelligenceEngine;
  private conversationHistory: { question: string; response: ExecutiveResponse }[] = [];
  ...
  async ask(question: string, context?: Record<string, unknown>): Promise<ExecutiveResponse> {
    this.setStatus('running');
    const query: ExecutiveQuery = { question, context };
  ...
  private async enrichResponse(response: ExecutiveResponse, question: string): Promise<ExecutiveResponse> {
  ...
  async summarizeDailyEvents(): Promise<ExecutiveResponse> {
    const dashboard = await this.engine.generateExecutiveDashboard();
    const alerts = await this.engine.getAlerts();
...
TOP PERFORMERS: ${dashboard.projectPerformance.topPerformers.join(', ')}`;
    ...
    const response: ExecutiveResponse = {
      answer: summary,
      evidence: [
  ...
  async prepareBoardMeeting(): Promise<ExecutiveResponse> {
    const dashboard = await this.engine.generateExecutiveDashboard();
    const health = await this.engine.getCompanyHealth();
...
7. RECOMMENDATIONS FOR BOARD
  ...
  private trendIndicator(current: number, baseline: number): string {
    const diff = current - baseline;
    if (diff > 10) return 'Strong growth';
    if (diff > 0) return 'Moderate improvement';
    if (diff > -10) return 'Stable';
    return 'Declining';
  }

  private healthTrend(): string {
  ...
  async analyzeProfitDrop(details?: string): Promise<ExecutiveResponse> {
    return this.ask('Why did profits drop this month?', { details });
  }

  async recommendInvestment(): Promise<ExecutiveResponse> {
    return this.ask('What is the best project to invest in?');
  }

  async identifyBudgetRisks(): Promise<ExecutiveResponse> {
    return this.ask('Which projects are at risk of budget overrun?');
  }

  async simulateSteelPriceIncrease(percentage: number = 15): Promise<ExecutiveResponse> {
    return this.ask(`What happens if steel prices rise ${percentage}%?`, { steelPriceIncrease: percentage });
  }

  getConversationHistory(): { question: string; response: ExecutiveResponse }[] {
