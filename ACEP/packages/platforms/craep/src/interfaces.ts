...
import {
  ...
  DigitalTwinRobotics, KnowledgeGraph, DecisionLog, RobotStatus,
  GeoLocation, RobotType, AutonomousEquipment
} from './types';
...
export interface IFleetManager {
  ...
  getAllEquipment(): Promise<{ id: string; type: AutonomousEquipment; model: string; status: RobotStatus }[]>;
  ...
  getMaintenanceSchedule(): Promise<{ equipmentId: string; nextMaintenance: string; type: string }[]>;
}

export interface IMissionPlanner {
  createMission(name: string, type: RobotType, path: { start: GeoLocation; waypoints: GeoLocation[]; end: GeoLocation }, priority: number): Promise<Mission>;
  ...
  validateMission(mission: Mission): Promise<{ valid: boolean; issues: string[] }>;
  estimateEnergy(robotId: string, path: { start: GeoLocation; waypoints: GeoLocation[]; end: GeoLocation }): Promise<number>;
}
...
export interface ICoordinationEngine {
  ...
  deconflict(planId: string): Promise<{ conflicts: number; resolved: number; remaining: number }>;
...
export interface ISafetyMonitor {
  ...
  getSafetyReport(): Promise<{ totalAlerts: number; criticalAlerts: number; resolvedAlerts: number; activeZones: number }>;
}
