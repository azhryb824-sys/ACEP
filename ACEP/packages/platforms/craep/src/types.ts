...
export enum ConstructionSubtype {
...
export enum WeldingSubtype {
  CuttingRobot = 'CuttingRobot',
  AssemblyRobot = 'AssemblyRobot'
}

export enum PaintingSubtype {
...
export enum AutonomousEquipment {
  Excavator = 'Excavator',
  Bulldozer = 'Bulldozer',
  Grader = 'Grader',
  Loader = 'Loader',
...
export interface MissionConstraints {
  ...
  weatherLimits?: { maxWind: number; maxRain: number; maxTemp: number };
...
export interface Mission {
  ...
  path: MissionPath;
  priority: number;
  time: { start: string; estimatedEnd: string; actualEnd?: string };
  energy: { estimated: number; actual?: number; efficiency?: number };
...
export interface DroneMission {
  id: string;
  droneId: string;
  inspection: { targetArea: GeoLocation[]; requiredSensors: string[]; detail: string };
  photography: { waypoints: GeoLocation[]; resolution: string; overlap: number };
  survey: { boundary: GeoLocation[]; resolution: string; accuracy: number };
  progressTracking: { interval: number; compareWith: string; generateReport: boolean };
  thermal: { target: string; temperatureRange: { min: number; max: number } };
  lidar: { density: number; classification: string[]; generateDEM: boolean };
  quantityMeasurement: { material: string; stockpileLocation: GeoLocation; unit: string };
