...
import { GISLayerType, DataSourceType, CoordinateSystem, MapProjection, EngineeringNodeType, SpatialRelationType, SpatialQueryType } from './types';
export class GISEngine extends BaseEngine {
    layers = new Map();
    layerData = new Map();
    demData = new Map();
    knowledgeGraph;
    ...
    async loadLayer(source, type, options) {
        ...
        const layer = {
            ...
            crs: options?.crs || CoordinateSystem.WGS84,
            projection: options?.projection || MapProjection.TransverseMercator,
    ...
    detectSourceType(source) {
        ...
        if (source.includes('lidar') || source.includes('laz'))
            return DataSourceType.LiDAR;
        if (source.includes('survey'))
            return DataSourceType.SurveyData;
