import {
  AssetHealthIndex, MaintenancePriority, HealthScoreDetails,
  WeightedFactor, ConditionMonitoringData, SmartAssetCard,
  FailureAnalysis, WorkOrder, AssetCondition
} from './types';

export class AssetHealthIndexCalculator {
  ...
  calculate(
    ...
    const overallScore = Math.round(
      operationScore * this.weights.operation +
  ...
  private calculateOperationScore(condition: AssetCondition): number {
    return (condition.mechanical * 0.35 + condition.electrical * 0.30 +
            condition.structural * 0.20 + condition.cosmetic * 0.15);
  }

  private calculateInspectionScore(condition: AssetCondition): number {
    return condition.overall;
  }

  private calculateInspectionFrequency(condition: AssetCondition): number {
    ...
    if (expectedInterval <= 0) return 50;
  ...
  private calculateOpenFindingsScore(workOrders: WorkOrder[]): number {
    const openCount = workOrders.filter(w => w.status === 'InProgress' || w.status === 'Assigned').length;
    if (openCount === 0) return 100;
    if (openCount <= 2) return 80;
    if (openCount <= 5) return 60;
    if (openCount <= 10) return 40;
    return 20;
  }

  private calculateSensorScore(data: ConditionMonitoringData[]): number {
    if (data.length === 0) return 50;
    const recent = data.slice(-10);
    const scores = recent.map(d => {
      let score = 100;
      if (d.temperature !== null && d.temperature > 80) score -= 20;
      if (d.vibration !== null && d.vibration > 10) score -= 25;
      if (d.noise !== null && d.noise > 85) score -= 15;
      if (d.pressure !== null && (d.pressure < 50 || d.pressure > 150)) score -= 15;
      if (d.errors !== null && d.errors > 0) score -= d.errors * 5;
      return Math.max(0, score);
    });
    return scores.reduce((a, b) => a + b, 0) / scores.length;
  }

  private calculateTemperatureScore(data: ConditionMonitoringData[]): number {
    if (data.length === 0) return 50;
    const recent = data.slice(-10);
    const temps = recent.filter(d => d.temperature !== null).map(d => d.temperature!);
    if (temps.length === 0) return 50;
    const avg = temps.reduce((a, b) => a + b, 0) / temps.length;
    if (avg <= 40) return 100;
    if (avg <= 55) return 80;
    if (avg <= 70) return 60;
    if (avg <= 85) return 40;
    return 20;
  ...
  private calculateVibrationScore(data: ConditionMonitoringData[]): number {
    if (data.length === 0) return 50;
    const recent = data.slice(-10);
    const vibs = recent.filter(d => d.vibration !== null).map(d => d.vibration!);
    if (vibs.length === 0) return 50;
  ...
  private calculatePressureScore(data: ConditionMonitoringData[]): number {
    if (data.length === 0) return 50;
    const recent = data.slice(-10);
    ...
    if (avg >= 90 && avg <= 110) return 100;
    if (avg >= 75 && avg <= 125) return 80;
    if (avg >= 60 && avg <= 140) return 60;
    if (avg >= 40 && avg <= 160) return 40;
    return 20;
  }

  private calculateEnergyScore(data: ConditionMonitoringData[]): number {
    if (data.length === 0) return 50;
    const recent = data.slice(-10);
    const energies = recent.filter(d => d.energy !== null).map(d => d.energy!);
    if (energies.length === 0) return 50;
    const trend = energies[energies.length - 1] / (energies[0] || 1);
    if (trend <= 1.05) return 100;
    if (trend <= 1.15) return 80;
    if (trend <= 1.30) return 60;
    if (trend <= 1.50) return 40;
    return 20;
  }

  private calculateErrorScore(data: ConditionMonitoringData[]): number {
    if (data.length === 0) return 50;
    const recent = data.slice(-10);
  ...
  private calculateFailureScore(failures: FailureAnalysis[]): number {
    ...
    if (recent.length <= 1) return 70;
    if (recent.length <= 3) return 50;
    if (recent.length <= 5) return 30;
    return 10;
  }

  private calculateFailureSeverityScore(failures: FailureAnalysis[]): number {
    if (failures.length === 0) return 100;
    const severities = failures.map(f => {
      const costs = f.costOfFailure || 0;
      if (costs > 100000) return 10;
      if (costs > 50000) return 30;
      if (costs > 10000) return 50;
      if (costs > 1000) return 70;
      return 90;
    });
    return severities.reduce((a, b) => a + b, 0) / severities.length;
  }

  private calculateDowntimeScore(failures: FailureAnalysis[]): number {
    if (failures.length === 0) return 100;
    const totalDowntime = failures.reduce((a, f) => a + f.downtimeHours, 0);
    if (totalDowntime <= 1) return 100;
    if (totalDowntime <= 8) return 80;
    if (totalDowntime <= 24) return 60;
    if (totalDowntime <= 72) return 40;
    return 20;
  }

  private calculateMaintenanceScore(workOrders: WorkOrder[]): number {
    return this.calculateMaintenanceCompliance(workOrders);
  }

  private calculateMaintenanceCompliance(workOrders: WorkOrder[]): number {
    const scheduled = workOrders.filter(w =>
      w.type === 'Preventive' || w.type === 'Inspection');
    if (scheduled.length === 0) return 50;
    const completed = scheduled.filter(w => w.status === 'Completed').length;
    return (completed / scheduled.length) * 100;
  }

  private calculateBacklogScore(workOrders: WorkOrder[]): number {
    const backlog = workOrders.filter(w =>
      w.status === 'Assigned' || w.status === 'InProgress').length;
    if (backlog === 0) return 100;
    if (backlog <= 3) return 80;
    if (backlog <= 6) return 60;
    if (backlog <= 10) return 40;
    return 20;
  }

  private calculateCostEfficiencyScore(workOrders: WorkOrder[]): number {
    const completed = workOrders.filter(w => w.status === 'Completed');
    if (completed.length === 0) return 50;
    const ratios = completed.map(w => {
      if (w.estimatedCost <= 0) return 1;
      return w.actualCost / w.estimatedCost;
    });
    const avg = ratios.reduce((a, b) => a + b, 0) / ratios.length;
    if (avg <= 1.0) return 100;
    if (avg <= 1.1) return 80;
    if (avg <= 1.2) return 60;
    if (avg <= 1.5) return 40;
    return 20;
  }

  private calculateAgeScore(installDate: string, designLife: number): number {
    const installed = new Date(installDate).getTime();
    const elapsedYears = (Date.now() - installed) / (1000 * 60 * 60 * 24 * 365);
    if (designLife <= 0) return 50;
    const lifeRatio = elapsedYears / designLife;
    if (lifeRatio <= 0.2) return 100;
    if (lifeRatio <= 0.4) return 85;
    if (lifeRatio <= 0.6) return 70;
    if (lifeRatio <= 0.8) return 55;
    if (lifeRatio <= 1.0) return 40;
    return 20;
  }

  private calculateWearScore(asset: SmartAssetCard): number {
  ...
  private calculateObsolescenceScore(asset: SmartAssetCard): number {
    ...
    if (years <= 12) return 40;
    return 20;
  }

  private calculatePerformanceScore(data: ConditionMonitoringData[], asset: SmartAssetCard): number {
    if (data.length === 0) return 50;
    const recent = data.slice(-10);
    const loads = recent.filter(d => d.load !== null).map(d => d.load!);
    if (loads.length === 0) return 50;
    const avgLoad = loads.reduce((a, b) => a + b, 0) / loads.length;
    if (avgLoad >= 60 && avgLoad <= 90) return 100;
    if (avgLoad >= 40 && avgLoad <= 100) return 80;
    if (avgLoad >= 20 || avgLoad <= 120) return 60;
    return 40;
  }

  private calculateAvailabilityScore(workOrders: WorkOrder[]): number {
    const completed = workOrders.filter(w => w.status === 'Completed');
  ...
  private calculateOutputQualityScore(data: ConditionMonitoringData[]): number {
    if (data.length === 0) return 50;
    const recent = data.slice(-10);
    const errors = recent.filter(d => d.errors !== null).map(d => d.errors!);
    if (errors.length === 0) return 50;
    const avgErrors = errors.reduce((a, b) => a + b, 0) / errors.length;
    if (avgErrors === 0) return 100;
    if (avgErrors <= 0.5) return 85;
    if (avgErrors <= 1) return 70;
    if (avgErrors <= 3) return 50;
    return 30;
  }

  private buildWeightedFactors(category: string, items: { name: string; score: number; weight: number }[]): WeightedFactor[] {
    return items.map(i => ({
      name: i.name,
  ...
  private determinePriority(score: number): MaintenancePriority {
    if (score >= 80) return MaintenancePriority.Routine;
    if (score >= 65) return MaintenancePriority.Scheduled;
    if (score >= 45) return MaintenancePriority.Urgent;
    if (score >= 25) return MaintenancePriority.Immediate;
    return MaintenancePriority.Replacement;
  }

  private generateRecommendation(score: number, priority: MaintenancePriority, asset: SmartAssetCard): string {
    if (score >= 80) return `Asset ${asset.id} is in excellent condition. Continue routine maintenance.`;
    if (score >= 65) return `Asset ${asset.id} is in good condition. Schedule standard preventive maintenance.`;
    if (score >= 45) return `Asset ${asset.id} requires attention. Urgent maintenance recommended. Degradation detected in key components.`;
    if (score >= 25) return `Asset ${asset.id} is in poor condition. Immediate intervention required. Consider major overhaul.`;
