import {
  ReliabilityMetrics, ReliabilityTrend, FailureAnalysis,
  WorkOrder, FailureCategory, FailureFactor
} from './types';
...
export class ReliabilityAnalyzer {
  ...
  analyzeTrend(failures: FailureAnalysis[], workOrders: WorkOrder[]): ReliabilityTrend {
    ...
    const sorted = [...failures].sort((a, b) =>
      new Date(a.failureDate).getTime() - new Date(b.failureDate).getTime());
    ...
    const firstAvailability = this.calculateAvailability(
      this.calculateMTBF(firstHalf, 8760 * half), firstMttr);
    const secondAvailability = this.calculateAvailability(
      this.calculateMTBF(secondHalf, 8760 * half), secondMttr);
  ...
  private calculateTrend(first: number, second: number, higherIsBetter: boolean): 'Improving' | 'Stable' | 'Degrading' {
    const threshold = 0.10;
    if (first === 0 && second === 0) return 'Stable';
    const ratio = first !== 0 ? (second - first) / Math.abs(first) : (second > 0 ? 1 : 0);
    if (Math.abs(ratio) <= threshold) return 'Stable';
    if (higherIsBetter) return ratio > 0 ? 'Improving' : 'Degrading';
  ...
  predictRemainingLife(
    ...
    designLife: number,
    installDate: string
  ): { estimatedRemainingYears: number; confidence: number; basedOn: string[]; recommendation: string } {
    ...
    const basedOn: string[] = ['Design Life Analysis'];
  ...
  classifyFailure(failure: FailureAnalysis): {
    ...
    const categories: { key: string; factors: FailureFactor[] }[] = [
  ...
  getFailureStatistics(failures: FailureAnalysis[]): {
    ...
    if (months.length >= 3) {
      ...
      if (recent > old * 1.2) failureRateTrend = 'Increasing';
      else if (recent < old * 0.8) failureRateTrend = 'Decreasing';
    }
