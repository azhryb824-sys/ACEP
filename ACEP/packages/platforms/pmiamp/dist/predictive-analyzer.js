...
class PredictiveAnalyzer {
    async analyzeFailurePatterns(assetId, failures) {
        const evidence = [];
        let failureProbability = 0.05;
        let severity = types_1.SeverityLevel.Negligible;
        let impact = types_1.OperationalImpact.None;
        const vulnerableComponents = [];
        if (failures.length === 0) {
            return this.buildAnalysis(assetId, 'Failure Patterns', failureProbability, 8760, 'hours', severity, impact, vulnerableComponents, 0.3, evidence, ['No historical failure data available']);
        }
        ...
        if (avgDowntime > 24) {
            ...
            impact = types_1.OperationalImpact.Shutdown;
            evidence.push({
            ...
            severity = types_1.SeverityLevel.Major;
            impact = types_1.OperationalImpact.Interrupted;
        }
        else if (avgDowntime > 1) {
            severity = types_1.SeverityLevel.Moderate;
            impact = types_1.OperationalImpact.Degraded;
        }
        ...
        for (const type of failureTypes) {
            const count = failures.filter(f => {
                const factors = f[type] || [];
                return factors.some((ff) => ff.contributor);
            }).length;
            if (count > failures.length * 0.3) {
                vulnerableComponents.push({
                    ...
                    criticality: count > failures.length * 0.5 ? types_1.AssetCriticality.Critical : types_1.AssetCriticality.High,
        ...
        return this.buildAnalysis(assetId, 'Failure Patterns', failureProbability, Math.round(8760 * (1 - failureProbability)), 'hours', severity, impact, vulnerableComponents, confidence, evidence, this.generateActions(failureProbability, severity));
    }
    async analyzeMaintenanceHistory(assetId, workOrders) {
        const evidence = [];
        let failureProbability = 0.05;
        if (workOrders.length === 0) {
            return this.buildAnalysis(assetId, 'Maintenance History', failureProbability, 8760, 'hours', types_1.SeverityLevel.Negligible, types_1.OperationalImpact.None, [], 0.2, evidence, ['No maintenance history recorded']);
        }
        ...
        return this.buildAnalysis(assetId, 'Maintenance History', failureProbability, Math.round(8760 * (1 - failureProbability)), 'hours', emergencyCount > 2 ? types_1.SeverityLevel.Major : types_1.SeverityLevel.Minor, emergencyCount > 2 ? types_1.OperationalImpact.Interrupted : types_1.OperationalImpact.Degraded, [], confidence, evidence, this.generateActions(failureProbability, emergencyCount > 2 ? types_1.SeverityLevel.Major : types_1.SeverityLevel.Minor));
    }
    async analyzeOperationalBehavior(assetId, sensorData) {
        const evidence = [];
        let failureProbability = 0.05;
        let severity = types_1.SeverityLevel.Negligible;
        const vulnerableComponents = [];
        if (sensorData.length < 5) {
            return this.buildAnalysis(assetId, 'Operational Behavior', failureProbability, 8760, 'hours', types_1.SeverityLevel.Negligible, types_1.OperationalImpact.None, [], 0.15, evidence, ['Insufficient operational data for analysis']);
        }
        ...
        if (avgTemp > 75) {
            ...
            vulnerableComponents.push({
                ...
                criticality: types_1.AssetCriticality.High, replacementCost: 15000
            });
        }
        ...
        if (avgVib > 8) {
            ...
            vulnerableComponents.push({
                ...
                criticality: types_1.AssetCriticality.Critical, replacementCost: 25000
            });
        }
        ...
        const impact = severity === types_1.SeverityLevel.Critical ? types_1.OperationalImpact.Shutdown :
            severity === types_1.SeverityLevel.Major ? types_1.OperationalImpact.Interrupted :
                types_1.OperationalImpact.Degraded;
        return this.buildAnalysis(assetId, 'Operational Behavior', failureProbability, Math.round(8760 * (1 - failureProbability)), 'hours', severity, impact, vulnerableComponents, confidence, evidence, this.generateActions(failureProbability, severity));
    }
    async analyzeManufacturerData(assetId, asset) {
        const evidence = [];
        ...
        return this.buildAnalysis(assetId, 'Manufacturer Data', failureProbability, Math.round(8760 * (1 - failureProbability)), 'hours', failureProbability > 0.5 ? types_1.SeverityLevel.Major : types_1.SeverityLevel.Minor, failureProbability > 0.5 ? types_1.OperationalImpact.Interrupted : types_1.OperationalImpact.Degraded, [], confidence, evidence, this.generateActions(failureProbability, failureProbability > 0.5 ? types_1.SeverityLevel.Major : types_1.SeverityLevel.Minor));
    }
    async analyzeSensorTrends(assetId, sensorData) {
        const evidence = [];
        let failureProbability = 0.05;
        const vulnerableComponents = [];
        if (sensorData.length < 10) {
            return this.buildAnalysis(assetId, 'Sensor Trends', failureProbability, 8760, 'hours', types_1.SeverityLevel.Negligible, types_1.OperationalImpact.None, [], 0.1, evidence, ['Insufficient sensor data for trend analysis']);
        }
        ...
        if (secondTemp > firstTemp * 1.15 && secondTemp > 65) {
            ...
            vulnerableComponents.push({
                ...
                estimatedLifeRemaining: 4, criticality: types_1.AssetCriticality.High,
        ...
        if (secondVib > firstVib * 1.2 && secondVib > 6) {
            ...
            vulnerableComponents.push({
                ...
                estimatedLifeRemaining: 2, criticality: types_1.AssetCriticality.Critical,
        ...
        failureProbability = Math.min(1, failureProbability);
        const dataQuality = sensorData.filter(d => d.temperature !== null || d.vibration !== null).length / sensorData.length;
        const confidence = Math.min(0.9, 0.3 + sensorData.length * 0.01) * dataQuality;
        return this.buildAnalysis(assetId, 'Sensor Trends', failureProbability, Math.round(8760 * (1 - failureProbability)), 'hours', failureProbability > 0.5 ? types_1.SeverityLevel.Major : types_1.SeverityLevel.Minor, failureProbability > 0.5 ? types_1.OperationalImpact.Interrupted : types_1.OperationalImpact.Degraded, vulnerableComponents, confidence, evidence, this.generateActions(failureProbability, failureProbability > 0.5 ? types_1.SeverityLevel.Major : types_1.SeverityLevel.Minor));
    }
    async analyzeEnvironmentalImpact(assetId, asset) {
        const evidence = [];
        let failureProbability = 0.05;
        ...
        return this.buildAnalysis(assetId, 'Environmental Impact', failureProbability, Math.round(8760 * (1 - failureProbability)), 'hours', failureProbability > 0.3 ? types_1.SeverityLevel.Moderate : types_1.SeverityLevel.Minor, failureProbability > 0.3 ? types_1.OperationalImpact.Degraded : types_1.OperationalImpact.None, [], confidence, evidence, this.generateActions(failureProbability, failureProbability > 0.3 ? types_1.SeverityLevel.Moderate : types_1.SeverityLevel.Minor));
    }
    async comprehensiveAnalysis(assetId, asset, sensorData, failures, workOrders) {
        ...
        const weightedFailureProb = analyses.reduce((s, a) => s + a.failureProbability * a.confidence, 0) / (totalWeight || 1);
        const weightedTtf = analyses.reduce((s, a) => s + a.estimatedTimeToFailure * a.confidence, 0) / (totalWeight || 1);
        const highestConfidence = analyses.reduce((best, a) => a.confidence > best.confidence ? a : best, analyses[0]);
    ...
    buildAnalysis(assetId, source, failureProbability, estimatedTimeToFailure, estimatedTimeUnit, severity, operationalImpact, vulnerableComponents, confidence, evidence, recommendedActions) {
        return {
            assetId,
            timestamp: new Date().toISOString(),
    ...
    assessManufacturerQuality(manufacturer) {
        ...
        if (budget.some(b => mfg.includes(b.toLowerCase()) || manufacturer.includes(b)))
            return 'Budget';
        if (premium.some(p => mfg.includes(p.toLowerCase())))
            return 'Premium';
        return 'Standard';
    }
    generateActions(failureProbability, severity) {
        const actions = [];
        ...
        if (severity === types_1.SeverityLevel.Critical || severity === types_1.SeverityLevel.Catastrophic) {
