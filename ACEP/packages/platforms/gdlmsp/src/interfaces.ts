...
import {
  CountryConfig, Language, TranslationEntry, CurrencyConfig, TaxRule,
...
export interface IGlobalEngine extends IEngine {
  ...
  calculateTax(amount: number, countryId: string, entityId: string): Promise<{ total: number; breakdown: TaxRule[] }>;
...
export interface ITaxEngine {
  ...
  calculate(amount: number, countryId: string, entityId: string): Promise<{ total: number; taxAmount: number; breakdown: TaxRule[] }>;
...
export interface IComplianceManager {
  ...
  checkCompliance(countryId: string, projectId: string): Promise<{ compliant: boolean; gaps: string[]; score: number }>;
