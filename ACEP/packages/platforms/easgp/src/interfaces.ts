...
export interface IAuthManager {
  authenticate(method: AuthMethod, credentials: Record<string, unknown>): Promise<{ token: string; identity: Identity }>;
  ...
  handleSSOCallback(provider: string, code: string): Promise<{ token: string; identity: Identity }>;
  registerFIDO2Credential(identityId: string, credential: Record<string, unknown>): Promise<void>;
  authenticateFIDO2(credentialId: string, signature: string): Promise<{ token: string; identity: Identity }>;
...
export interface IRiskManager {
  ...
  getRiskSummary(): Promise<{ total: number; critical: number; high: number; medium: number; low: number; mitigated: number }>;
}
