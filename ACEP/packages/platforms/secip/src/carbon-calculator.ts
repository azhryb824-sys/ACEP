...
export const DEFAULT_EMISSION_FACTORS: Record<EmissionSource, number> = {
  [EmissionSource.Cement]: 0.92,
  [EmissionSource.Concrete]: 0.15,
  [EmissionSource.Steel]: 1.85,
  [EmissionSource.Aluminum]: 2.5,
  [EmissionSource.Glass]: 0.85,
  [EmissionSource.Brick]: 0.24,
  [EmissionSource.Paint]: 3.2,
  [EmissionSource.Transportation]: 0.41,
  [EmissionSource.Equipment]: 0.35,
  [EmissionSource.Electricity]: 0.45,
  [EmissionSource.Fuel]: 2.68,
  [EmissionSource.Operations]: 0.28,
  [EmissionSource.Maintenance]: 0.19
};

export const SCOPE_CLASSIFICATION: Record<EmissionSource, 1 | 2 | 3> = {
  [EmissionSource.Cement]: 1,
  [EmissionSource.Concrete]: 1,
  [EmissionSource.Steel]: 1,
  [EmissionSource.Aluminum]: 1,
  [EmissionSource.Glass]: 1,
  [EmissionSource.Brick]: 1,
  [EmissionSource.Paint]: 3,
  [EmissionSource.Transportation]: 1,
  [EmissionSource.Equipment]: 1,
  [EmissionSource.Electricity]: 2,
  [EmissionSource.Fuel]: 1,
  [EmissionSource.Operations]: 3,
  [EmissionSource.Maintenance]: 3
};

const EMISSION_FACTOR_MAP: Record<EmissionSource, EmissionFactors> = {} as Record<EmissionSource, EmissionFactors>;

for (const source of Object.values(EmissionSource)) {
  EMISSION_FACTOR_MAP[source] = {
    source,
    co2Factor: DEFAULT_EMISSION_FACTORS[source] * 0.95,
    ch4Factor: DEFAULT_EMISSION_FACTORS[source] * 0.03,
    n2oFactor: DEFAULT_EMISSION_FACTORS[source] * 0.02,
    co2eFactor: DEFAULT_EMISSION_FACTORS[source],
    unit: "kgCO2e/kg",
    confidence: 0.85,
...
export class CarbonCalculator implements ICarbonCalculator {
  private factors: Record<EmissionSource, EmissionFactors>;
  ...
  constructor(customFactors?: Partial<Record<EmissionSource, Partial<EmissionFactors>>>) {
    ...
    if (customFactors) {
      for (const [source, factor] of Object.entries(customFactors)) {
        if (this.factors[source as EmissionSource]) {
          Object.assign(this.factors[source as EmissionSource], factor);
        }
      }
    }
  }
  ...
  async calculate(data: {
    sources: Array<{ source: EmissionSource; activity: number; unit: string }>;
  }): Promise<CarbonFootprint> {
    const records: EmissionRecord[] = [];
    const bySource: Record<string, number> = {};
    const scope1Sources: EmissionRecord[] = [];
    const scope2Sources: EmissionRecord[] = [];
    const scope3Sources: EmissionRecord[] = [];
    ...
    for (const entry of data.sources) {
      ...
      const scope = SCOPE_CLASSIFICATION[entry.source];
      ...
      if (scope === 1) scope1Sources.push(record);
      else if (scope === 2) scope2Sources.push(record);
      else scope3Sources.push(record);
    }
  ...
  async calculateScope1(sources: EmissionRecord[]): Promise<{ total: number; sources: EmissionRecord[] }> {
    const scope1Source = sources.filter(s => SCOPE_CLASSIFICATION[s.source] === 1);
    const total = scope1Source.reduce((s, r) => s + r.co2e, 0);
    return { total, sources: scope1Source };
  }

  async calculateScope2(electricityConsumption: number, gridFactor: number): Promise<{ total: number; sources: EmissionRecord[] }> {
    const factor = this.factors[EmissionSource.Electricity];
  ...
  async calculateScope3(supplyChainData: Record<string, number>): Promise<{
    total: number;
    categories: Record<string, number>;
    sources: EmissionRecord[];
  }> {
    const sources: EmissionRecord[] = [];
    const categories: Record<string, number> = {};
    let total = 0;

    for (const [category, amount] of Object.entries(supplyChainData)) {
      const avgFactor = Object.values(DEFAULT_EMISSION_FACTORS).reduce((a, b) => a + b, 0) / Object.keys(DEFAULT_EMISSION_FACTORS).length;
      const co2e = amount * avgFactor * 0.5;
      total += co2e;
      categories[category] = (categories[category] || 0) + co2e;
      sources.push({
        source: EmissionSource.Operations,
        activity: amount,
        unit: "kg",
        factors: this.factors[EmissionSource.Operations],
        co2e,
        scope: 3,
        category,
  ...
  async getEmissionFactor(source: EmissionSource): Promise<number> {
  ...
  async createNetZeroPlan(data: unknown): Promise<NetZeroPlan> {
    const input = data as { currentEmissions?: number; targetYear?: number; baselineYear?: number } | undefined;
  ...
  async offsetEmissions(amount: number, type: string): Promise<{ cost: number; verified: boolean }> {
  ...
  private getCategory(source: EmissionSource): string {
    const categories: Record<EmissionSource, string> = {
      [EmissionSource.Cement]: "Materials",
      [EmissionSource.Concrete]: "Materials",
      [EmissionSource.Steel]: "Materials",
      [EmissionSource.Aluminum]: "Materials",
      [EmissionSource.Glass]: "Materials",
      [EmissionSource.Brick]: "Materials",
      [EmissionSource.Paint]: "Materials",
      [EmissionSource.Transportation]: "Logistics",
      [EmissionSource.Equipment]: "Equipment",
      [EmissionSource.Electricity]: "Energy",
      [EmissionSource.Fuel]: "Energy",
      [EmissionSource.Operations]: "Operations",
      [EmissionSource.Maintenance]: "Operations"
    };
