...
export class CADParserService {
  ...
  extractElements(cad: { layers: CADLayer[]; entities: CADEntity[] }): Record<string, { name: string; count: number; estimatedArea: number }> {
    this.ensureInitialized();
    const grouped: Record<string, { name: string; count: number; estimatedArea: number }> = {};
    for (const layer of cad.layers) {
      const mapped = LAYER_MAP[layer.name] || { element: 'Unknown', category: 'Other' };
      const key = mapped.category;
      if (!grouped[key]) grouped[key] = { name: key, count: 0, estimatedArea: 0 };
  ...
  convertToBuildingModel(cad: { layers: CADLayer[]; entities: CADEntity[] }, area: number = 500, floors: number = 2): VirtualBuilding {
    this.ensureInitialized();
    const layersByName: Record<string, CADLayer> = {};
    for (const l of cad.layers) layersByName[l.name] = l;
  ...
  private _fromJson(json: Record<string, unknown>): { layers: CADLayer[]; entities: CADEntity[] } {
    return {
      layers: (json.layers as CADLayer[]) || [],
      entities: (json.entities as CADEntity[]) || [],
    };
  }
  ...
  private _mockCad(_data: Buffer | string | object): { layers: CADLayer[]; entities: CADEntity[] } {
    const layerNames = ['A-WALL', 'A-DOOR', 'A-GLAZ', 'A-DIMS', 'S-COLS', 'S-SLAB', 'S-BEAM', 'S-FOOT', 'P-PIPE', 'E-COND', 'E-LITE', 'M-DUCT'];
    const layers: CADLayer[] = layerNames.map((name, i) => ({
      name,
