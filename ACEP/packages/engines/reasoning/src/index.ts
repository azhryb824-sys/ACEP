import { BaseEngine, IReasoningEngine, ProjectFacts, ReasoningTrace, RuleAction, FactType } from '@acep/core';
import { KnowledgeGraph, RuleEngine, RulesRegistry } from '@acep/knowledge-base';

export class EngineeringReasoningEngine extends BaseEngine implements IReasoningEngine {
  ...
  async reason(facts: ProjectFacts, knowledge: unknown): Promise<ReasoningTrace[]> {
  ...
  async buildReasoningTree(facts: ProjectFacts): Promise<unknown> {
    ...
    if (facts.spaces) {
      for (const [spaceType, space] of Object.entries(facts.spaces)) {
        ...
        (tree.nodes as unknown[]).push(node);
      }
    }
  ...
  async detectLogicalErrors(facts: ProjectFacts): Promise<unknown[]> {
  ...
  async suggestAlternatives(context: unknown): Promise<unknown[]> {
  ...
  async calculateConfidenceScore(facts: ProjectFacts): Promise<{ overall: number; breakdown: Record<string, number> }> {
    const breakdown: Record<string, number> = {
      projectType: facts.projectType.confidence,
      spaces: Object.values(facts.spaces || {}).reduce((sum: number, s: any) => sum + (s.confidence || 0.5), 0) / (Object.keys(facts.spaces || {}).length || 1),
  ...
  private calculateCompleteness(facts: ProjectFacts): number {
    let score = 0;
    let total = 0;
    
    if (facts.projectType.value) { score += 1; total += 1; }
    if (facts.floors) { score += 1; total += 1; }
    if (facts.spaces && Object.keys(facts.spaces).length > 0) { score += 1; total += 1; }
    if (facts.landArea) { score += 1; total += 1; }
    if (facts.builtArea) { score += 1; total += 1; }
    if (facts.location) { score += 1; total += 1; }
