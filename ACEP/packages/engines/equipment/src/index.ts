import { BaseEngine, IEquipmentEngine, BOQDocument, EquipmentRequirement, EquipmentSchedule, EquipmentCategory } from '@acep/core';
import { KnowledgeGraph, EquipmentLibrary, BOQItemsLibrary } from '@acep/knowledge-base';

export class EquipmentEngine extends BaseEngine implements IEquipmentEngine {
  ...
  async determineEquipment(boq: BOQDocument): Promise<EquipmentRequirement[]> {
    ...
    for (const item of boq.items) {
      ...
      for (const equipId of template.equipmentRequired) {
        if (!categoryTotals[equipId]) categoryTotals[equipId] = 0;
    ...
    for (const [equipId, totalQty] of Object.entries(categoryTotals)) {
      ...
      const req: EquipmentRequirement = {
        ...
        category: equipRecord.category as EquipmentCategory,
        count,
        hours,
  ...
  async selectEquipment(activity: unknown, constraints: unknown): Promise<unknown> {
    const cons = constraints as Record<string, unknown> || {};
    const budget = (cons.budget as number) || 100000;
    const duration = (cons.duration as number) || 30;

    const options = this.equipmentLib.findByCategory((activity as any)?.category || 'Excavation');
    return options
      .map(o => ({ ...o, totalCost: o.dailyCost * duration, score: (o.availability * 10) - (o.dailyCost * duration / budget) }))
      .sort((a, b) => b.score - a.score);
  }

  async calculateUtilization(equipment: EquipmentRequirement[]): Promise<number> {
    if (equipment.length === 0) return 0;
    return Math.round(equipment.reduce((s, e) => s + e.utilization, 0) / equipment.length * 100) / 100;
  }

  async compareOptions(equipmentId: string): Promise<unknown[]> {
  ...
  async scheduleEquipment(requirements: EquipmentRequirement[]): Promise<EquipmentRequirement[]> {
  ...
  private createDefaultRequirement(): EquipmentRequirement {
    return {
      equipmentId: 'eq-default-001',
      name: 'General Construction Equipment',
      category: EquipmentCategory.Transportation,
      count: 1,
      hours: 40,
      dailyCost: 800,
      totalCost: 4000,
