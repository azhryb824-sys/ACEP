function registerUETSRoutes(app, uets) {
  ...
  app.post('/api/v1/uets/training/enqueue', (req, res) => {
    ...
    const job = uets.enqueueTraining(models, options || {});
    res.status(201).json(job);
  });

  app.post('/api/v1/uets/retraining/enqueue', (req, res) => {
    const { reason, options } = req.body;
    const job = uets.enqueueRetraining(reason || 'manual', options || {});
    res.status(201).json(job);
  });
  ...
  app.post('/api/v1/uets/jsonl/write', (req, res) => {
    try {
      ...
      const outputDir = (req.body && req.body.outputDir) || path.join(__dirname, '..', '..', '..', 'data', 'uets', 'jsonl');
      const limit = (req.body && req.body.limit) ? parseInt(req.body.limit) : 500;
