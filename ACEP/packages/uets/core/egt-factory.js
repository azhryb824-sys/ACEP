...
class EGTFactory {
  ...
  fromCSVProject(row) {
    ...
    const egt = new EngineeringGroundTruth({
      source: 'csv',
      originalId: row.project_id || `csv-${Date.now()}-${Math.random().toString(36).substr(2, 5)}`,
      description: {
      ...
      cost: {
        ...
        perM2: area > 0 ? parseFloat(row.estimated_cost_sar || 0) / area : 0
      },
      schedule: {
  ...
  fromCSVRisk(row) {
    return {
      ...
      probability: parseFloat(row.probability || 0.3),
      impact: parseFloat(row.impact || 0.5),
      riskScore: parseFloat(row.probability || 0.3) * parseFloat(row.impact || 0.5),
  ...
  fromCSVQualityDefect(row) {
    return {
      ...
      severity: parseFloat(row.severity || 2),
