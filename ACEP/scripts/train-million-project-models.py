...
DEFAULT_ARTIFACT = ROOT / "models" / "registry" / "candidates" / "acep-million-synthetic-v1.json"
DEFAULT_REPORT = ROOT / "docs" / "governance" / "ACEP_MILLION_SYNTHETIC_TRAINING.md"
DEFAULT_OUTPUT = ROOT / ".runtime" / "model-training" / "acep-million-synthetic-v1"
...
class CorpusGenerator:
    ...
    def record(self, raw: dict, y: np.ndarray, index: int, split: str) -> dict:
        ...
        return {
            ...
            "geometry": {
                "grossBuiltAreaM2": round(float(raw["grossArea"][index]), 2),
                "footprintAreaM2": round(float(raw["footprint"][index]), 2),
                "landAreaM2": round(float(raw["landArea"][index]), 2),
                "floors": int(raw["floors"][index]),
                "basements": int(raw["basements"][index]),
                "buildings": int(raw["buildings"][index]),
            },
            "capacity": round(float(raw["capacity"][index]), 2),
            "targets": {name: round(float(y[index, target_index]), 5) for target_index, name in enumerate(TARGETS)},
...
def main() -> int:
    ...
    for shard_number, start in enumerate(range(0, args.records, args.shard_size), start=1):
        ...
        if not args.no_record_files:
            ...
            shard_hash = hashlib.sha256(f"not-persisted:{args.seed}:{start}:{count}".encode()).hexdigest()
            shard_size = 0
    ...
    dataset_manifest = {
        "schema": "acep-synthetic-project-record/v1",
    ...
    artifact = {
        "schemaVersion": "1.0.0",
        "modelId": "acep-million-synthetic-v1",
