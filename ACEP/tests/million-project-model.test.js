...
const artifactPath = path.join(__dirname, '..', 'models', 'registry', 'candidates', 'acep-million-synthetic-v1.json');
...
describe('million-project research candidate', () => {
  ...
  test('changes detailed BOQ quantities with project scale and withholds building finishes from roads', () => {
    const large = ai.quantityEstimator.estimateBOQ('Hospital', 58000 / 6, 6, 'specialized', 'Riyadh', {
      ...
      landArea: 42000, basements: 2, buildings: 3, capacity: 200, method: 'design_build'
    });
    ...
    expect(road.summary.status).toBe('experimental_conceptual_only');
    expect(road.summary.totalCost).toBeNull();
    expect(road.items.map(item => item.code)).toEqual(expect.arrayContaining(['SCP-001', 'CON-REF', 'STL-REF']));
    expect(road.items.some(item => /لياسة|دهان|بلاط داخلي/.test(item.description))).toBe(false);
  });
  ...
  test('connects the candidate to cost, schedule, risk, and quality without false certainty', () => {
    const input = {
      ...
      basements: 2, buildings: 3, capacity: 200, method: 'design_build', city: 'الرياض'
    };
    const cost = ai.costEstimator.estimateCost([], 'Hospital', 58000 / 6, 6, 'specialized', 'الرياض', {
      ...input, grossBuiltArea: 58000
    });
    const schedule = ai.scheduleOptimizer.generateSchedule('Hospital', 58000, 6, 'specialized', 'الرياض', 'design_build', input);
    ...
    expect(new Set(risks.risks.map(risk => `${risk.probability}:${risk.impact}`)).size).toBeGreaterThan(3);
    expect(quality.qualityScore).toBeLessThan(99);
    expect(quality.estimatedDefects).toBeGreaterThan(0);
