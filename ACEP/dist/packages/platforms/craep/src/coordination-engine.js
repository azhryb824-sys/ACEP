...
class CoordinationEngine extends core_1.BaseEngine {
    plans = new Map();
    swarms = new Map();
        ...
        for (let i = 0; i < plan.robots.length; i++) {
            for (let j = i + 1; j < plan.robots.length; j++) {
                ...
                if (dist < 10) {
                    conflicts++;
                    const rule = {
                        id: (0, uuid_1.v4)(), robotIds: [r1.id, r2.id],
    ...
    async preventCollisions(planId) {
        ...
        for (let i = 0; i < plan.robots.length; i++) {
            for (let j = i + 1; j < plan.robots.length; j++) {
                const r1 = this.robots.get(plan.robots[i]);
                const r2 = this.robots.get(plan.robots[j]);
                if (!r1 || !r2 || r1.status !== types_1.RobotStatus.OnMission || r2.status !== types_1.RobotStatus.OnMission)
                    continue;
                const dist = this.calculateDistance(r1.location, r2.location);
                const velocity1 = 5;
                const velocity2 = 5;
        ...
        for (const zone of this.safetyZones.values()) {
            ...
            for (const robotId of plan.robots) {
                ...
                if (!robot || robot.status !== types_1.RobotStatus.OnMission)
                    continue;
    ...
    async optimizeSwarm(swarmConfig) {
        ...
        const activeRobots = swarmConfig.robotIds.filter(id => {
            ...
            return r && r.status === types_1.RobotStatus.OnMission;
        });
    ...
    async manageHumanRobotCollaboration(collaboration) {
