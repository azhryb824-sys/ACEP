import { ComputerVisionDetection, DetectionType, EmergencyPlan, HazardSource, IncidentRecord, Investigation, InvestigationType, Location, Permit, RiskAssessment, RiskType, SafetyMetrics, SafetyReport, EnterpriseSafetyIndex, SafetyKnowledgeEntry, SeverityLevel } from "./types";
...
export interface IRiskAnalyzer {
    analyzeRisk(riskType: RiskType, probability: number, severity: number, affectedPeople: number, responseTime: number, currentControl: number): number;
...
export interface IIncidentManager {
    ...
    getIncidentTrends(projectId: string, start: Date, end: Date): Promise<IncidentTrendEntry[]>;
    getIncidentStatistics(projectId: string): Promise<IncidentStatistics>;
}
export interface IInvestigationSupport {
    createInvestigation(incidentId: string, type: InvestigationType, lead: string): Promise<Investigation>;
    addFinding(investigationId: string, finding: string): Promise<Investigation>;
    completeInvestigation(investigationId: string): Promise<Investigation>;
