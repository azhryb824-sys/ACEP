...
exports.SaudiBuildingCodePlugin = void 0;
...
class SaudiBuildingCodePlugin extends index_1.PluginBase {
    engineId = 'saudi-building-code';
    name = 'Saudi Building Code Engine';
    _rules = [];
    _materials = [];
    _laborData = [];
    constructor() {
    ...
    async process(input) {
        ...
        try {
            ...
            const sbcVersion = this.context.config.sbcVersion || '2023';
            const strictMode = this.context.config.strictMode ?? true;
            const region = this.context.config.region || 'central';
            traces.push({
                step: 2,
    ...
    _checkStructuralRequirements(data, traces) {
        const warnings = [];
        const result = {};
        // Check concrete grade
        const concreteGrade = data.concreteGrade;
        if (concreteGrade) {
        ...
        // Check seismic zone
        const seismicZone = data.seismicZone;
    ...
    _checkMaterialCompliance(data, region, traces) {
        const warnings = [];
        const result = {};
        ...
        if (materials && Array.isArray(materials)) {
            ...
            result.approvedCount = materials.filter((m) => approvedMaterials.includes(m.id)).length;
        }
        traces.push({
            step: 4,
    ...
    _generateComplianceReport(data, sbcVersion) {
        const violations = [];
        const passed = [];
        const notChecked = [];
    ...
    _simulateRuleCheck(ruleId, _data) {
...
exports.SaudiBuildingCodePlugin = SaudiBuildingCodePlugin;
// ─── Plugin Factory ───────────────────────────────────────────────────────
