...
exports.NegotiationAgent = void 0;
...
class NegotiationAgent {
    ...
    generateMockSuppliers(materialId, materialName, quantity, location) {
        return [
            {
                supplierId: 'sup-001',
    ...
    async compareSuppliers(materialId, quantity, location) {
    ...
    calculateWeightedScores(options, weights) {
    ...
    async generateComparisonMatrix(items) {
    ...
    async suggestBestOption(options) {
    ...
    async prepareRFQ(materialList) {
        this.logger.info(`Preparing RFQ for ${materialList.length} items`);
        const items = materialList.map((item, index) => ({
            lineNumber: index + 1,
            code: item.code,
...
exports.NegotiationAgent = NegotiationAgent;
