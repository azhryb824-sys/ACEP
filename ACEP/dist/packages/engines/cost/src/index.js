...
class CostEngine extends core_1.BaseEngine {
    knowledgeGraph;
    materialLib;
    laborLib;
    equipmentLib;
    constructor(kg, matLib, labLib, eqLib) {
    ...
    async getPrice(materialId, location) {
    ...
    async compareSuppliers(materialId) {
        const material = this.materialLib.get(materialId);
        if (!material)
            return [];
        return [
    ...
    async sensitivityAnalysis(cost) {
        const scenarios = [
