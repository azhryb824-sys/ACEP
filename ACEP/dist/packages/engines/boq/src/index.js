...
exports.BOQEngine = void 0;
const core_1 = require("@acep/core");
class BOQEngine extends core_1.BaseEngine {
    knowledgeGraph;
    boqItemsLib;
    spacesLib;
    materialLib;
    constructor(kg, boqLib, spLib, matLib) {
    ...
    async generateBOQ(building) {
        ...
        for (const template of allTemplates) {
            const hasRequiredSpace = template.requiredSpaces.length === 0 ||
                template.requiredSpaces.some(rs => spaceTypes.has(rs));
            const hasOptionalSpace = template.optionalSpaces.length === 0 ||
                template.optionalSpaces.some(os => spaceTypes.has(os));
            if (!hasRequiredSpace && !hasOptionalSpace)
                continue;
            const matchedSpaces = building.spaces.filter(s => template.requiredSpaces.includes(s.type) ||
                template.optionalSpaces.includes(s.type));
            let quantity = 0;
            ...
            const item = {
                ...
                level: hasRequiredSpace ? core_1.BOQItemLevel.Required : core_1.BOQItemLevel.Optional,
                unit: template.unit,
        ...
        const doc = {
            ...
            metadata: {
                ...
                status: core_1.ExecutionStatus.BOQGeneration,
                version: '1.0.0',
    ...
    async getTemplateForSpace(spaceType) {
        return this.boqItemsLib.findBySpace(spaceType);
    }
    async mergeSimilarItems(items) {
    ...
    async validateDependencies(items) {
        const boqItems = items;
    ...
    async detectMissingItems(items) {
        const boqItems = items;
    ...
    estimateQuantity(template, space, building) {
    ...
    estimateDefaultQuantity(template, building) {
    ...
    calculateItemConfidence(template, matchedSpaces, material) {
        let confidence = 0.5;
    ...
    calculateCorrectionFactors(template, building) {
        const factors = [];
