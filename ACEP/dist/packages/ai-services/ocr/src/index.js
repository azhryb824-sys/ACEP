...
class OCRService {
    config = { provider: 'mock', baseUrl: 'http://localhost:3000', tesseractLang: 'ara+eng' };
    initialized = false;
    ...
    ensureInitialized() {
        if (!this.initialized)
            throw new Error('OCRService not initialized. Call initialize() first.');
    }
}
