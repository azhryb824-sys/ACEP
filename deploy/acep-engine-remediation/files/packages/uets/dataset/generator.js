...
const { writeFileAtomic } = require('../../runtime/atomic-json-store');
...
class UETSDatasetGenerator {
    ...
    for (const egt of egts) {
      ...
      for (const defect of (egt.quality && egt.quality.defectTypes || [])) {
        qualityDefects.push({
          ...
          severity: ({ 3: 'Critical', 2: 'High', 1: 'Medium', 0.5: 'Low' })[defect.severity] || 'Unverified',
  ...
  generateJSONLTrainingData(egts, domain) {
    ...
    for (const egt of egts) {
      ...
      if (sample) samples.push({
        ...sample,
        projectId: egt.uuid,
        provenance: egt.source || 'unknown',
        dataStatus: 'unverified',
        suitableForModelApproval: false,
        areaBasis: egt.geometry.areaBasis || 'unverified',
        groupId: egt.originalId || egt.uuid
      });
    }
    return samples;
  }
  ...
  writeJSONLToDisk(outputDir, egts, options = {}) {
    ...
    const splitRatio = Number(options.validationRatio ?? 0.1);
    if (!Number.isFinite(splitRatio) || splitRatio <= 0 || splitRatio >= 1) throw new Error('validationRatio must be between 0 and 1');
    const seed = String(options.seed || 'acep-uets-v1');
    ...
    for (const [domain, samples] of Object.entries(domains)) {
      ...
      for (const sample of samples) {
        const fingerprint = crypto.createHash('sha256').update(JSON.stringify([sample.system, sample.instruction, sample.output])).digest('hex');
        if (seen.has(fingerprint)) continue;
        seen.add(fingerprint);
        const digest = crypto.createHash('sha256').update(`${seed}:${sample.groupId}`).digest();
        const score = digest.readUInt32BE(0) / 0x100000000;
        (score < splitRatio ? validation : train).push(sample);
      }

      const write = (file, rows) => {
        writeFileAtomic(file, rows.length ? rows.map(r => JSON.stringify(r)).join('\n') + '\n' : '');
        return rows.length;
      };

      written[domain] = {
        train: write(path.join(dir, 'training.jsonl'), train),
        validation: write(path.join(dir, 'validation.jsonl'), validation),
      };
    }

    console.log(`[UETS-Gen] Wrote ${Object.keys(written).length} JSONL domains to ${outputDir}`);
    return { domains: Object.keys(written).length, samples: written, seed, groupBy: 'projectId' };
  }
  ...
  _buildJSONLSample(egt, domain) {
    const id = `${domain}-${crypto.createHash('sha256').update(String(egt.uuid)).digest('hex').slice(0,24)}`;
    const type = egt.classification.projectType;
    const area = egt.geometry.totalArea;
    const floors = egt.geometry.floors;
    const city = egt.location.city;
    ...
    switch (domain) {
      case 'engineering_llm':
        return {
          ...
          source: 'UETS-EGT-v1', created: this.generatedAt || new Date().toISOString()
        };
      ...
      case 'cost_ai':
        if (egt.geometry.areaBasis !== 'gross_floor_area' || !Number.isFinite(area) || area <= 0 || !Number.isFinite(egt.cost.total) || egt.cost.total <= 0) return null;
        return {
          id, domain,
          system: 'أنت خبير في تقدير تكاليف مشاريع البناء.',
          instruction: `قدر التكلفة الإجمالية لمشروع ${type} بمساحة مبنية إجمالية لجميع الأدوار ${area} م² و ${floors} أدوار.`,
          output: `التكلفة التقديرية: ${egt.cost.total} ريال سعودي. تكلفة المتر المربع من إجمالي المساحة المبنية: ${(egt.cost.total / area).toFixed(4)} ريال.`,
          cost: { totalSar: egt.cost.total, grossFloorAreaM2: area, rateSarPerGrossM2: egt.cost.total / area },
          source: 'UETS-EGT-v1', created: this.generatedAt || new Date().toISOString()
        };
      ...
      case 'risk_ai':
        return {
          ...
          output: `عدد المخاطر المحددة: ${egt.risks.length}. المخاطر الرئيسية: ${egt.risks.slice(0, 3).map(r => `${r.category} (مؤشر ترتيبي اصطناعي؛ ليس احتمال حادث)`).join('، ')}`,
          source: 'UETS-EGT-v1', created: this.generatedAt || new Date().toISOString()
        };
      ...
      case 'quality_ai':
        return {
          ...
          output: `عدد سجلات العيوب في المصدر غير المتحقق منه: ${(egt.quality && egt.quality.expectedDefects) ?? 0}. نقاط فحص بحثية: ${(egt.quality && egt.quality.defectTypes || []).map(d => d.type).join('، ') || 'لا تتوفر سجلات'}`,
          source: 'UETS-EGT-v1', created: this.generatedAt || new Date().toISOString()
        };
