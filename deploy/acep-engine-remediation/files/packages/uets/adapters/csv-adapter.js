...
class CSVAdapter {
  ...
  migrateAllToEGT() {
    ...
    for (const proj of projects) {
      const originalId = proj.project_id || `csv-${crypto.randomUUID()}`;
  ...
  _loadCSV(filename) {
    ...
    try {
      ...
      const [headers, ...rows] = parseRows(fs.readFileSync(filePath, 'utf8'));
      return rows.map(values => Object.fromEntries(headers.map((key, i) => [key.trim(), values[i].trim()])));
    } catch (e) {
      throw new Error(`CSV source ${filename} is invalid: ${e.message}`);
    }
  }
