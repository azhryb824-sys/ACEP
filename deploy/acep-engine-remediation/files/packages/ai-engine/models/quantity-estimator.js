 ...
 * 2. Training data (85,062 unverified legacy BOQ items) provides unit prices per category
...
const { buildConceptEstimate } = require('../engineering-concept-estimator');
...
class QuantityEstimator {
  ...
  async train(csvPath) {
    ...
    this.boqData = readRows(fp).map(l => {
      const c = l;
      return {
  ...
  estimateBOQ(projectType, area, floors, finishing = 'Standard', region = 'Riyadh', extraParams = {}) {
    ...
    // A governed research prediction is converted into a project-family-specific
    // concept cost plan.  This replaces the previous mix of detailed-looking
    // building rows and four generic infrastructure references.  The result is
    // internally reconciled, but remains explicitly unsuitable for procurement.
    if (trainedPrediction.available) {
      return buildConceptEstimate(trainedPrediction, {
        ...extraParams,
        region,
        finishing
      });
    ...
    if (trainedPrediction.blocked) return this._conceptualBOQ(params, extraParams, trainedPrediction, finishing, region);
