...
const { assessModelDomain } = require('../model-domain-guards');

const CONFIG_PATH = path.join(__dirname, '..', 'model-training', 'project-archetypes.json');
const DEFAULT_ARTIFACT_PATH = path.join(__dirname, '..', '..', '..', 'models', 'registry', 'candidates', 'acep-million-synthetic-v2.json');
...
class MillionProjectModel {
  ...
  resolveCity(value) {
    ...
    return Object.keys(this.config.cities).find(city => city.toLowerCase() === lowered) || null;
  }

  predict(input = {}) {
    if (!this.artifact) return { available: false, blocked: true, reason: this.error || 'artifact_not_loaded' };
    ...
    if (!profile) return { available: false, blocked: true, reason: 'unsupported_project_type' };
    ...
    if (!grossBuiltArea || !floors) return { available: false, blocked: true, reason: 'gross_area_and_floors_required' };

    const cityName = this.resolveCity(input.city);
    if (!cityName) return { available: false, blocked: true, reason: 'unsupported_city', issues: [{ field: 'city', message: 'The location has no calibrated reference; select a supported city or provide reviewed local data.' }] };
    const finishName = canonical(input.finishing) || 'standard';
    const methodName = canonical(input.method) || (profile.family === 'existing' ? 'rehabilitation' : 'traditional');
    if (!this.config.finishes[finishName]) return { available: false, blocked: true, reason: 'unsupported_finishing' };
    if (!this.config.methods[methodName]) return { available: false, blocked: true, reason: 'unsupported_construction_method' };
    const domainIssues = assessModelDomain({ ...input, method: methodName }, profile, this.config);
    if (domainIssues.length) return { available: false, blocked: true, reason: domainIssues[0].code, issues: domainIssues };
    const assumptions = [];
    if (!input.finishing) assumptions.push({ field: 'finishing', value: finishName, source: 'declared_default', requiresConfirmation: true });
    if (!input.method) assumptions.push({ field: 'constructionMethod', value: methodName, source: 'declared_default', requiresConfirmation: true });
    for (const field of ['landArea', 'footprintArea', 'capacity', 'complexity', 'buildings', 'basements']) {
      if (input[field] === undefined || input[field] === null) assumptions.push({ field, source: 'synthetic_prior', requiresConfirmation: true });
    }
    ...
    for (const target of this.artifact.targets) {
      ...
      if (target === 'riskScore') {
        // APE is relative error. It must not be used as an absolute score margin.
        const delta = Number((model.calibrationByProjectType?.[profile.key] || model.calibration)?.p90AbsoluteError);
        intervals[target] = Number.isFinite(delta) ? { lower: Math.max(0, value - delta), upper: Math.min(1, value + delta), basis: 'synthetic_calibration_absolute_error', nominalCoverage: 0.9 } : null;
      } else {
        const delta = Number((model.calibrationByProjectType?.[profile.key] || model.calibration || model.holdout)?.p90Ape ?? 0.25);
        intervals[target] = delta < 1 ? { lower: Math.max(0, value / (1 + delta)), upper: value / (1 - delta), basis: 'synthetic_relative_error_inversion', nominalCoverage: 0.9, realWorldCalibrated: false } : null;
      }
    }
    ...
    return {
      ...
      intervals,
      assumptions,
      applicability: { status: 'within_declared_model_domain', physicalDesignApproved: false },
