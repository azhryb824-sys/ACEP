const { parseRows } = require('./csv-rows');
/**
...
class ProjectAnalyzer {
  ...
  async train(csvPath) {
    ...
    const lines = parseRows(content).slice(1);
  ...
  analyzeProject(description, options = {}) {
    ...
    const detectedType = kb.detectProjectType(description);
    const requestedType = typeof options.type === 'string' ? options.type.trim() : '';
    const fallbackMap = {
      'Apartment_Finishing': 'Apartment_Building',
    };
    // Prefer a controlled structured type when the knowledge base or an
    // explicit, documented training fallback supports it. Narrative detection
    // remains the fallback for legacy unstructured callers.
    const requestedTypeSupported = Boolean(
      requestedType && (kb.getProjectType(requestedType) || fallbackMap[requestedType])
    );
    let type = requestedTypeSupported ? requestedType : detectedType;
    ...
    if (trainedPrediction.blocked) return { insufficientData: true, message: trainedPrediction.reason, missing: ['supported_model_domain'], projectType: type };
