...
async function main() {
  ...
  try {
    ai.knowledgeEngine.recordProject(
      { id: 'synthetic-legacy-test-project', metadata: { name: 'Synthetic test fixture' }, extracted: { type: 'Villa', area: 100, floors: 1 } },
      { items: [] },
      {},
      { projectType: { primary: 'Villa' } }
    );
  ...
  try {
    ...
    let blocked=false;try { await rOrch.retrainAll(); } catch(error) { blocked=/UETS is required/.test(error.message); }
    check('missing UETS blocks bundled-data fallback',blocked);
    const {UnifiedTrainingSystem}=require('../../uets');
    const {EGTFactory}=require('../../uets/core/egt-factory');
    const uets=new UnifiedTrainingSystem().initialize();
    uets.addEGT(new EGTFactory().fromCSVProject({project_id:'retrain-regression',project_type:'Villa',building_area_m2:200,floors:2,estimated_cost_sar:800000,year:2024}));
    rOrch.setUETS(uets);
    const r1 = await rOrch.retrainAll({models:['projectAnalyzer']});
    check('retrainAll consumes UETS source',r1.executionMode==='uets_immutable_snapshot');
    check('selected model trained',r1.models===1);
    check('source snapshot is bound',/^snapshot-[a-f0-9]{64}$/.test(r1.snapshotId));
    check('candidate remains inactive',r1.activated===false);
    const r2 = await rOrch.retrainAll({models:['projectAnalyzer']});
    check('retraining can repeat',r2.models===1);
    check('repeat uses same immutable source',r1.snapshotId===r2.snapshotId);
    check('repeat returns actual source record count',r2.totalRecords===r1.totalRecords&&r2.totalRecords>0);
    const r3 = await rOrch.retrainWeakAreas({models:['projectAnalyzer']});
    check('weak-area candidate trained',r3.models===1);
    check('weak areas are not self-certified',r3.weakAreasResolved===false);
    check('candidate requires independent evaluation',r3.suitableForModelApproval===false);
    check('completed retrains recorded',rOrch.getStats().totalRetrains===3);
