...
class WorkflowEngine {
  ...
  async stepExtraction(project, description, parseDescriptionFn) {
    ...
    project.setExtracted({
      ...
      capacityUnit: params.capacityUnit || null,
  ...
  async stepAnalysis(project, description) {
    ...
    const analysis = this.ai.projectAnalyzer.analyzeProject(description, {
      ...
      capacityUnit: effective.capacityUnit,
      city: effective.city,
  ...
  async stepBOQ(project, description) {
    ...
    const extra = {
      ...
      inclusions: effective.inclusions,
  ...
  async stepCost(project) {
    if (project.boq.summary?.status === 'blocked') {
      const blocked = { status: 'blocked', totalCost: null, costPerM2: null, reason: project.boq.summary.reason, requiresHumanReview: true };
      project.setCost(blocked);
      return blocked;
    }
    const effective = this._getEffectiveParams(project);
  ...
  async stepSchedule(project) {
    const scope = project.boq.summary?.scope;
    if (project.boq.summary?.status === 'blocked' || scope?.excludedPackages?.length) {
      const schedule = { status: 'blocked', reason: 'scoped_resource_schedule_required', totalDuration: null, totalMonths: null,
        activities: (project.boq.items || []).map((item, i) => ({ id: `S${i + 1}`, name: item.description, packageCode: item.code, duration: null, predecessors: [] })),
        criticalPath: [], scope, requiresHumanReview: true,
        limitations: ['A full-project duration cannot be reused for a reduced scope. Quantified resources, dependencies and a working calendar are required.'] };
    ...
    const scheduleData = this.ai.scheduleOptimizer.generateSchedule(
      ...
      effective.finishing || 'Standard', effective.city || 'Riyadh', effective.method || undefined, {
        landArea: effective.landArea,
        footprintArea: effective.footprintArea,
        basements: effective.basements,
        buildings: effective.buildings,
  ...
  _getEffectiveParams(project) {
    return {
      ...
      length: project.extracted.length ?? null,
      width: project.extracted.width ?? null,
      ...
      capacityUnit: project.extracted.capacityUnit || null,
      ...
      systems: project.extracted.systems || [],
      inclusions: project.extracted.inclusions || '',
      exclusions: project.extracted.exclusions || '',
    };
  }
