...
class AIOrchestrator extends WorkflowEngine {
  ...
  async runFullPipeline(project, description, parseDescriptionFn) {
    ...
    for (const stepName of MANDATORY_SEQUENCE) {
      ...
      stepResult.durationMs = Date.now() - stepStartedAt;
    ...
    try {
      ...
      switch (stepName) {
        ...
        case 'boq': {
          ...
          return { name: stepName, status: boq.summary?.status === 'blocked' ? 'blocked' : 'completed', error: boq.summary?.reason, data: boq.summary };
        }
        case 'cost': {
          const cost = await this.stepCost(project);
          return { name: stepName, status: cost.status === 'blocked' ? 'blocked' : 'completed', error: cost.reason, data: { totalCost: cost.totalCost } };
        }
        case 'schedule': {
          const schedule = await this.stepSchedule(project);
          return { name: stepName, status: schedule.status === 'blocked' ? 'blocked' : 'completed', error: schedule.reason, data: { totalDays: schedule.totalDuration } };
        }
        case 'risks': {
      ...
      if (process.env.ACEP_DEBUG_PIPELINE_ERRORS === 'true') {
        console.error(`[ACEP pipeline:${stepName}]`, err.stack || err);
      }
  ...
  async _step3DNavigation(project) {
    ...
    const overlayData = {
      ...
      risk: project.risks.riskLevel || 'Unassessed',
  ...
  _cvCostVsRisk(project, currentStep) {
    ...
    const riskLevel = project.risks.riskLevel || 'Unassessed';
